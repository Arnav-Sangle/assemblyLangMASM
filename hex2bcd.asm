...
msg6 db 10,13, 'Equivalent BCD Number: $'
msg7 db 10,13, 'Enter 5 Digit BCD Number: $'
msg8 db 10,13, 'Equivalent HEX Number: $'
...
menu: disp msg1     ;call macro disp to display menu
      ...
      cmp al,31h    ;compare choice acceptede from user with 31=1
...
hex:  disp msg5     ;call macro disp enter 4 digit hex number
        call input  ;call input proc for hex to accept 4 digit no. from user
...
a1:    cmp al,41h   ;check with hex alphabet A
       ...
       sub al,37h   ;if letter accepted to Uppercase sub 37
       jmp insert
...
a2:    cmp al,61h   ;check lowercase a
       ...
       jg invalid
       sub al,57h   ;substract 57 if letter is lowercase
       jmp insert
...
hexp proc
       mov ax,bx
       mov bx,0ah   ;load decimal 0010 in bx fro division
       mov cl,00    ;clear counter

b1:    mov dx,00h   ;clear
         div bx     ;div by 10 and store the result in DX:AX
...
c1:
       ...
       jg invalid1
       sub al,30h   ;ASCII conversion
       mov [si],al  ;mov read val from al to score index
       inc si
